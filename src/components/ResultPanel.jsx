...
export default function ResultPanel({ result, originalCss, onDownload, onGenerateJson, isGenerating, onSectionUpdate }) {
  if (!result) return (
    <div style={{
      display: 'flex',
      flexDirection: 'column',
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20, height: '100%' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10, flex: 1, overflowY: 'auto' }}>
        {result.sections.map((section, i) => (
          <SectionCard key={section.id || i} section={section} index={i} originalCss={originalCss} onUpdate={onSectionUpdate} />
        ))}
      </div>
